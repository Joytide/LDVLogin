...
**EN**
I am not responsible for the misuse of this script, it isn't in any case a tool
to be used to commit attendance fraud.
If you are in my school, check the 8.2 section of our Internal Rules for more
informations on attendance fraud.
...
user_agent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/87.0.4280.66 Safari/537.36"
...
class Course:
            ...
            if i == 0:
                ...
                try:
                    self.releve_link = (
                        ...
                        + re.findall(
                            r"(href=\")(/student/presences/[0-9]+)\"", str(els[i])
                        )[0][1]
                    )
                    self.id = re.findall(
                        r"(href=\")(/student/presences/[0-9]+)\"", str(els[i])
                ...
                try:
                    self.zoom_link = re.findall(
                        r"(https://devinci-online\.zoom\.us/.+?pwd=[a-zA-Z0-9]+)(\")",
                        str(els[i]),
                    )[0][0]
                except Exception as e:
                    logger.error(e)
    ...
    def set_presence(self, session):
        ...
        res = session.post(
            "https://www.leonard-de-vinci.net/student/presences/upload.php", data=d
        )
        return res
...
def login_status(session):
    ...
    if res:
        if "Mot de passe" in res.text:
            return False
        elif "Occupation des salles" in res.text:
            return True
        else:
...
def login(session):
    ...
    res = session.post(
        "https://www.leonard-de-vinci.net/ajax.inc.php",
        data="act=ident_analyse&login=" + user,
        headers={"Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"},
    )
    try:
        lssop_endpoint = re.findall(
            r"\/lssop\/[a-z0-9]*/.*@edu\.devinci\.fr", res.text
        )[0]
    except Exception as e:
        logger.error(e)
        ...
        exit(0)
    res = session.get(
        "https://www.leonard-de-vinci.net" + lssop_endpoint, allow_redirects=False
    )
    SAMLRequest = re.findall(r"(\?SAMLRequest=)(.*)\">", res.text)[0][1]
    res = session.post(
        "https://adfs.devinci.fr/adfs/ls?SAMLRequest="
        + SAMLRequest.replace("&amp;", "&"),
        data={
            "UserName": user,
            "Password": passwd,
    ...
    try:
        lssop_endpoint_2 = re.findall(
            r"(name=\"RelayState\" value=\")(https://www\.leonard-de-vinci\.net/lssop/[0-9a-z]*)\"",
            res.text,
        )[0][1]
    except Exception as e:
        logger.error(e)
    ...
    SAMLResponse = re.findall(
        r"(name=\"SAMLResponse\" value=\")(.*)(\" /><input)", res.text
    )[0][1]
